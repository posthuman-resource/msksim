...
function flattenInventory(inv: Inventory): [string, string, string, number][] {
  ...
  return out.sort((a, b) => `${a[0]}|${a[1]}|${a[2]}`.localeCompare(`${b[0]}|${b[1]}|${b[2]}`));
}
...
describe('bootstrapExperiment', () => {
  ...
  it('is deterministic across two calls with the same seed', () => {
    ...
    expect(r1.world1.agents.map((a) => a.position)).toEqual(
      r2.world1.agents.map((a) => a.position),
    );
    ...
    // Repeat for world2.
    expect(r1.world2.agents.map((a) => a.position)).toEqual(
      r2.world2.agents.map((a) => a.position),
    );
  });

...
describe("createDefaultPolicy", () => {
  ...
  it("produces bit-identical output sequences from two RNGs with the same seed", () => {
    ...
    const seqA = Array.from({ length: 100 }, () =>
      policy({ speaker, hearer, rng: rngA }),
    );
    const seqB = Array.from({ length: 100 }, () =>
      policy({ speaker, hearer, rng: rngB }),
    );
  ...
  it("honors renamed language labels and never returns the hardcoded default strings", () => {
    ...
    expect(
      policy({ speaker: makeAgent("W1-Bi"), hearer: makeAgent("W1-Mono"), rng }),
    ).toBe(firstLang);
    ...
    expect(
      policy({ speaker: makeAgent("W1-Bi"), hearer: makeAgent("W1-Bi"), rng }),
    ).toBe(firstLang);
  });
  ...
  it("works with the full defaultLanguagePolicies matrix from defaults.ts", () => {
    const policy = createDefaultPolicy(
      makePolicyConfig(defaultLanguagePolicies),
    );
    const rng = createRNG(0);
  ...
  it("inventorySet helper builds an inventory readable by the test fixtures", () => {
    ...
    const inv = inventorySet(emptyInventory(), L1, ref, lex, 2.0 as import("@/lib/schema/primitives").Weight);

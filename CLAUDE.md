...
- The drizzle client is a **lazy-init singleton** exported from `lib/db/client.ts` via a Proxy. The database connection opens on the first method call (e.g. `db.select()`), not at module load. Similarly, `env` in `lib/env.ts` validates on first property access. This prevents build-time crashes on Render.com (and any CI) where the database and `MSKSIM_SESSION_SECRET` are unavailable during `next build`. **Do not convert these back to eager module-level initialization** — that is a build-breaking regression guarded by `tests/build-safety.test.ts`. Do not instantiate `drizzle(...)` or `new Database(...)` elsewhere. Tests use `:memory:` by setting `MSKSIM_DB_PATH=':memory:'` **before** importing the module (use `vi.stubEnv`).
...
- The DB path and session secret flow through `lib/env.ts` (Zod-validated lazily on first property access via Proxy). Scripts outside Next use `@next/env`'s `loadEnvConfig(process.cwd())` before importing any module that transitively imports `lib/env.ts`. Because esbuild/tsx hoists static `import` statements above executable code, use an **async wrapper + dynamic `import()`** for the DB client in scripts (see `scripts/migrate.ts`).
...
- **Recharts SSR boundary**: every file that imports from `'recharts'` starts with `'use client'`. Recharts depends on `ResizeObserver` and SVG layout during mount; SSR execution throws. A grep for `"import.*recharts"` in files **without** `'use client'` is a smoke signal for an SSR leak.
...
Established in step 24:

- `SimulationWorkerApi` gained `updateConfig(partial: Partial<ExperimentConfig>): Promise<void>` in step 24. The worker validates the partial via `ExperimentConfig.partial().parse(...)`, merges shallowly into `state.simState.config`, and returns. The engine re-reads `state.simState.config` on every tick, so live parameter changes take effect on the next `step()` call without losing time-series history. Not all parameters are live-safe: `world1/2.monolingualBilingualRatio` and `seed` change initial conditions and require `api.reset()` + `api.init()` to apply correctly. Callers use `handleRebootstrap` (in `simulation-shell.tsx`) for reset-required changes.
...
- `crypto.subtle.digest` requires a **secure context** — HTTPS or `localhost`. `next start` on `http://localhost:$PORT` qualifies for the localhost exception so MCP verification works, but production deployments must be behind HTTPS or the config-hash helper and export filename derivation will throw. (`app/(auth)/playground/config-hash.ts` has no fallback by design.) (_Rationale_: `crypto.subtle` is undefined in non-secure contexts; a silent fallback would produce non-deterministic or missing export filenames and break the reproducibility invariant.)
- Debouncing worker side effects with `useDeferredValue` is a footgun: React docs state explicitly that `useDeferredValue` does not prevent side effects, only defers rendering. For slider → `api.updateConfig()` communication use `useState` + `useEffect` + `setTimeout` with cleanup; see `app/(auth)/playground/controls-panel.tsx` for the canonical pattern. (_Rationale_: `useDeferredValue` with a delayed rendering wouldn't stop `postMessage` calls on every drag frame.)

...
- The playground controls panel (`controls-panel.tsx`) splits config knobs into **live-safe** (debounced 300ms via `useState + useEffect + setTimeout`, applied through `onConfigUpdate`) and **reset-required** (applied through `onRebootstrap` after `worker.reset() + init()`). Step 35 added `gaussianSigma` and `attractThreshold` as live-safe sliders, conditionally rendered on `successPolicy.kind === 'gaussian'` and `movement.enabled` respectively. Their integer step counts (`attractStep`, `repelStep`, `gaussianTopK`) and the `enabled` toggles stay in the config editor — toggling them mid-run produces conceptually confusing dynamics. New live-safe sliders should follow the same five-piece pattern: draft state, sync-from-props effect, debounced commit effect, conditional render, and a no-op early return inside the commit effect when the gating field is off.

...
- Lattice topologies in `lib/sim/topology/lattice.ts` default to **open (non-toroidal) boundaries**. Corner cells have fewer neighbors than interior cells (3 vs 8 for Moore, 2 vs 4 for Von Neumann). Pass `toroidal: true` to `LatticeTopology` for periodic boundaries. Tests that assume uniform neighbor counts will miscount on corners and edges. (*Rationale*: open boundaries match the source PDF's visual representation and the geometry cited in Lu, Korniss, Szymanski 2008; see `docs/plan/10-topology-implementations.md` §4.)
